#!/usr/bin/python
""" Source:
https://raw.githubusercontent.com/nickpack/Igor/master/soco/soco.py
"""
...
if __name__ == '__main__':
    ...
    try:
        ...
        raise SystemExit('Missing player')
    if player not in players:
        raise SystemExit('Unrecognized player.  Choices={0}'.format(
                         players.keys()))

    def _get_soco(player_name):
        soco = SoCo(players[player])
        return soco

    soco = _get_soco(player)
    if mode == 'mute':
        soco.mute(True)
        print('mutting volume')
    elif mode == 'unmute':
        soco.mute(False)
        ...
        print(soco.get_speaker_info()['uid'])
    elif mode == 'join':
        try:
